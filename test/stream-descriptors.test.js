...
describe('unevalStream descriptor holes', () => {
	...
	function assert_descriptor_diagnostic(error, context, bad, root, path) {
		...
		expect(error.message).toMatch(
			new RegExp(`${context.replace(/[()]/g, '\\$&')}, template hole 1: received an object`)
		);
	...
	test('never reads unknown descriptor fields and composes captured controls into operations', async () => {
		...
		let reads = 0;
		const result = await unevalStream(
			{ job, keepalive: keepalive.promise },
			(value, js) => {
				if (value !== job) return;
				const descriptor = {
					type: 'async-value',
	...
	test('retains custom sequence controls through next and completion', async () => {
		...
		const iterator = {
			pulls: 0,
			next() {
				return ++this.pulls === 1 ? first.promise : complete.promise;
			}
		};
		const source = {
		...
		const result = await unevalStream(
			...
			(value, js) =>
				value === job && {
					...
					next: ({ target, control }, outcome) =>
						js`${target}.events.push(${control}===${target}.control,${outcome})`,
					complete: ({ target, control }, outcome) =>
						js`${target}.events.push(${control}===${target}.control,${outcome})`,
					error: () => js``
				},
			{ id }
		...
		const result = await unevalStream(
			...
			(value, js) =>
				value === job && {
					...
					source: ready.promise,
					construct(capture) {
						capture(js`[${new Promise(() => {})}]`);
						return js`({value:null})`;
					},
					resolve(reference, outcome) {
						control = reference.control;
						return js`${reference.target}.value=${outcome}`;
					},
					reject: () => js``
				},
	...
	test('populates sparse and null-object holes before repeated descriptor construction in every value region', async () => {
		...
		for (const mode of ['head', 'folded', 'outcome']) {
			...
			const value =
				mode === 'head' ? graph : mode === 'folded' ? Promise.resolve(graph) : gate.promise;
			let constructions = 0;
		...
		const result = await unevalStream(
			...
			(value, js) =>
				value instanceof Job && {
					...
					resolve: ({ target }) =>
						value === outer
							? js`${target}.values=[${sparse_child},${null_child},${nested},${nested}]`
							: js``,
					reject: () => js``
				},
			...
			(value, js) =>
				value === job && {
					...
					resolve: ({ target }, payload) =>
						js`${target}.values=[${shared},${shared},${payload}];${target}.get=()=>${shared}`,
					reject: () => js``
				},
		...
		const result = await unevalStream(
			...
			(value, js) =>
				value instanceof Job && {
					...
					construct: (capture) =>
						js`({name:${value.name},control:${capture(js`[]`)},child:${value.child ?? null}})`,
					resolve: ({ target, control }) =>
						js`${target}.control_matched=(${control}===${target}.control)`,
					reject: () => js``
				},
	...
	test('preserves descriptor diagnostics nested in an asynchronous payload region', async () => {
		...
		expect(reports.length).toBe(1);
		assert_descriptor_diagnostic(
			reports[0],
			'async descriptor construct()',
			bad,
			outcome.promise,
			'.job.deep.bad'
		);
	...
	for (const phase of ['resolve', 'next', 'complete']) {
		test(`preserves operation diagnostics and redacts recovered ${phase} failures from client source`, async () => {
			...
			expect(block.value).not.toMatch(
				/invalid_descriptor_value|Cannot stringify a function|\.deep\.bad/
			);
			target.block(block.value);
			expect(reports.length).toBe(1);
			assert_descriptor_diagnostic(
				reports[0],
				`async descriptor ${phase}()`,
				bad,
				job,
				'.deep.bad'
			);
	...
	for (const mode of ['reject', 'error', 'fallback reject', 'fallback error']) {
		test(`preserves finalized diagnostics for terminal ${mode} holes`, async () => {
			...
			const context = fallback
				? `async descriptor fallback ${sequence ? 'error' : 'reject'}()`
				: `async descriptor ${sequence ? 'error' : 'reject'}()`;
	...
	test('keeps sequential descriptor unwind paths independent', async () => {
		...
		const result = await unevalStream(
			...
			(value, js) =>
				value instanceof Job && {
					type: 'async-value',
					source: gates[value.index].promise,
					construct: () => js`({})`,
	...
	test('preserves hostile thrown values without inspecting them', async () => {
		...
		Object.defineProperty(throwing_message, 'message', {
			get() {
		...
		Object.defineProperty(hostile_conversion, 'message', {
			value: {
				[Symbol.toPrimitive]() {
					throw new Error('message converted');
				}
			}
		});
		...
		revoked.revoke();
		const hostile_cause = {};
		Object.defineProperty(hostile_cause, 'cause', {
			get() {
		...
		for (let i = 0; i < reasons.length; i++) {
			...
			expect(reports[0].message).toMatch(
				/async descriptor resolve\(\), template hole 1: received an object/
			);
			expect(Object.hasOwn(reports[0], 'cause')).toBeTruthy();
			expect(reports[0].cause).toBe(reason);
		}
	...
	test('does not reuse reported Symbol ownership while lowering descriptor holes', async () => {
		for (const mode of ['writable', 'frozen', 'hostile message']) {
			...
			expect(reports[1].message).toMatch(
				/async descriptor resolve\(\), template hole 1: received an object/
			);
	...
	for (const mode of ['construct', 'capture']) {
		test(`preserves an external cause during initial descriptor ${mode}`, async () => {
			...
			expect(error.message).toMatch(
				new RegExp(`async descriptor ${mode}\\(\\), template hole 1: received an object`)
			);
			...
			unevalStream(
				...
				(value, js) =>
					value instanceof Job && {
						type: 'async-value',
						source: value.ready,
						construct: () => js`({child:${value.child}})`,
						resolve: () => js``,
						reject: () => js``
					}
			)
