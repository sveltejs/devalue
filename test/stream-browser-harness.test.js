...
describe('browser smoke harness', () => {
	...
	async function no_profiles(directory, preserved = []) {
		...
		expect(entries.filter((entry) => entry.startsWith('devalue-browser-')).sort()).toEqual(preserved.sort());
	}
	...
	async function process_evidence(path, signal) {
		...
		expect(Number.isSafeInteger(evidence.pid) && evidence.pid > 0).toBeTruthy();
		expect(evidence.exit).toEqual({ code: null, signal });
		expect(() => process.kill(evidence.pid, 0)).toThrow(/ESRCH/);
		return evidence.pid;
	}
	...
	test('reports a missing browser from any cwd and removes only its created profile', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome failed to start: .*ENOENT/);
			expect(result.stderr).toMatch(/browser smoke cleanup failed: Error: Chrome process exit was not observed/);
	...
	test('reports an immediate browser exit without waiting for a watchdog', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome exited early \(code 7\)/);
			await no_profiles(parent);
		} finally {
			await rm(parent, { recursive: true, force: true });
		}
	...
	test('reports an early server exit without launching Chrome', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/browser smoke server (closed stdout before listening|exited early \(code 7\))/);
			expect(result.stderr).not.toMatch(/Chrome failed to start/);
			await no_profiles(parent);
		} finally {
			await rm(parent, { recursive: true, force: true });
		}
	...
	for (const setup_delay of [0, 500]) test(`waits for ${setup_delay}ms stubborn browser setup before force-terminating only its child`, async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr, result.stderr).toMatch(/Chrome DevTools endpoint timed out/);
			const pid = Number(await readFile(pid_file, 'utf8'));
			expect(Number(await readFile(ready_file, 'utf8'))).toBe(pid);
			expect(Number(await readFile(sigterm_file, 'utf8'))).toBe(pid);
			expect(() => process.kill(pid, 0)).toThrow(/ESRCH/);
			expect(await readFile(sentinel_file, 'utf8')).toBe('caller-owned');
	...
	test('times out in readiness before setup without requiring child-written evidence', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome test readiness timed out after 300ms/);
	...
	test('times out in the setup phase when setup is never acknowledged', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome test setup timed out after 300ms/);
			expect(result.stderr).not.toMatch(/test readiness timed out|DevTools endpoint timed out/);
			await process_evidence(process_file, 'SIGTERM');
			await absent(setup_file);
			await absent(ready_file);
			await no_profiles(parent);
	...
	for (const [label, setup_delay] of [['immediate', 0], ['configured', inherited_setup_delay]]) {
		test(`times out in readiness after ${label} setup and observes forced child exit`, async () => {
			...
			try {
				...
				expect(result.status, result.stderr || result.stdout).toBe(1);
				expect(result.stderr).toMatch(/Chrome test readiness timed out after 300ms/);
				expect(result.stderr).not.toMatch(/test setup timed out|DevTools endpoint timed out/);
				const parent_pid = await process_evidence(process_file, 'SIGKILL');
				expect(Number(await readFile(pid_file, 'utf8'))).toBe(parent_pid);
				expect(Number(await readFile(setup_file, 'utf8'))).toBe(parent_pid);
				expect(Number(await readFile(sigterm_file, 'utf8'))).toBe(parent_pid);
				await absent(ready_file);
				await no_profiles(parent);
			} finally {
				await rm(parent, { recursive: true, force: true });
	...
	test('reports a process evidence write failure as a cleanup diagnostic under an existing primary failure', async () => {
		...
		try {
			...
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome test readiness timed out after 300ms/);
			expect(result.stderr).not.toMatch(/Chrome process exit was not observed/);
			expect(result.stderr).toMatch(/browser smoke cleanup failed: Error: E(NOTDIR|NOENT|ACCES)/);
			expect(await readFile(blocker, 'utf8')).toBe('caller-owned');
			await no_profiles(parent);
		} finally {
			await rm(parent, { recursive: true, force: true });
		}
	...
	test('reports an early browser exit during readiness and removes its profile', async () => {
		...
		try {
			...
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/Chrome exited early \(code 7\)/);
			expect(result.stderr).not.toMatch(/test readiness timed out/);
			await no_profiles(parent);
		} finally {
			await rm(parent, { recursive: true, force: true });
		}
	...
	test('reports CDP disconnection without waiting for the harness watchdog', async () => {
		...
		try {
			...
			expect(result.signal, result.error?.message).toBe(null);
			expect(result.status, result.stderr || result.stdout).toBe(1);
			expect(result.stderr).toMatch(/CDP (disconnected|WebSocket closed before opening)/);
			await no_profiles(parent);
		} finally {
			await rm(parent, { recursive: true, force: true });
		}
