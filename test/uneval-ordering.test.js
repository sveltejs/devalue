...
describe('uneval: collection construction order', () => {
	describe.each(['map value', 'map key', 'set'])('%s:', (kind) => {
		describe.each(['first', 'middle', 'last'])('%s,', (position) => {
			test.each(['wrapper', 'container', 'shared wrapper', 'shared container'])(
				'%s',
				(root_kind) => {
					class Wrapper {
						static calls = 0;
					...
					expect(
						Array.from(revived.container, (entry) => (kind === 'set' ? entry : Array.from(entry)))
					).toEqual(
						Array.from(container, (entry) => (kind === 'set' ? replace(entry) : entry.map(replace)))
					);
	...
	test.each(['Map', 'Set'])(
		'initializes the ready prefix and chains entries between constructors (%s)',
		(kind) => {
			class Wrapper {
				constructor(container) {
					this.container = container;
	...
	test.each(['Map', 'Set'])(
		'constructs a complete shared collection from its buffered entries (%s)',
		(kind) => {
	...
	test.each(['wrapper', 'map', 'set'])(
		'populates both cyclic dependencies before constructing their shared owner (start=%s)',
		(start) => {
			class Wrapper {
	...
	test.each(['key', 'value', 'map'])(
		'constructs dependent custom Map keys and values before inserting the entry (start=%s)',
		(start) => {
			class Key {
				constructor(map) {
					this.map = map;
			...
			const [, revived_map, revived_key, revived_value] = vm.runInNewContext(source, {
				Key,
				Value
			});
	...
	test.each([false, true])(
		'preserves ordered prefixes through inline and named mixed containers (shared=%s)',
		(shared) => {
			class Wrapper {
				static calls = 0;
	...
	test.each([Map, Set].map((Constructor) => ({ Constructor })))(
		'does not append a chain to an allocation for an empty $Constructor.name',
		({ Constructor }) => {
	...
	test.each(['self', 'peer'])(
		'exposes completed earlier cycles to later constructors (%s)',
		(kind) => {
			class Snapshot {
				constructor(map) {
					this.map = map;
			...
			expect(kind === 'self' ? result.map.get('self') : result.map.get('peer').get('back')).toBe(
				result.map
			);
		}
	);
