...
export default graph;`;
	...
	function createWorker() {
		return new Worker(new URL('../lib/stream.worker.ts', import.meta.url), { type: 'module' });
	}
	function run() {
		worker?.terminate();
		runId++;
	...
	<main>
		<header class="topbar">
			<div class="brand">
				...
				<div>
					<h1>unevalStream</h1>
					<p>executable graph transport · protocol lab</p>
				</div>
			</div>
			<div class="header-stats">
				<Badge variant="outline">worker isolate</Badge><span>{blocks.length} blocks</span><span
					>{blockBytes} bytes</span
			...
			<section class="panel editor-panel" aria-label="Source editor">
				<div class="panel-header">
					<div class="file-label">
						<span class="ts-icon">TS</span><strong>example.ts</strong><span
							class="dirty-dot"
							aria-label="Editable"
						></span>
					</div>
					<div class="actions">
						...
						<Button size="sm" onclick={run} disabled={running}>▶ Run <kbd>⌘↵</kbd></Button>
					</div>
				</div>
				<div class="editor-wrap"><CodeEditor bind:value={source} onrun={run} /></div>
				<footer class="editor-footer">
					<span>TypeScript · ES2022</span><span>export default graph · optional replacer</span><span
						>isolated worker</span
					>
				</footer>
			</section>
					...
					<div class="result-header">
						<Tabs.List variant="line"
							><Tabs.Trigger value="stream"
								>Stream <Badge variant="secondary">{blocks.length}</Badge></Tabs.Trigger
							><Tabs.Trigger value="object"
								>Object graph {#if snapshot}<span class="live-dot"></span>{/if}</Tabs.Trigger
							></Tabs.List
						>
						<div class="result-header-tools">
							<span class="protocol">chronological · source before eval</span>
							<div class="display-toggle" role="group" aria-label="Stream source display mode">
								<Button
									variant={displayMode === 'pretty' ? 'secondary' : 'ghost'}
									size="xs"
									aria-pressed={displayMode === 'pretty'}
									onclick={() => (displayMode = 'pretty')}>Pretty</Button
								><Button
									variant={displayMode === 'raw' ? 'secondary' : 'ghost'}
									size="xs"
									aria-pressed={displayMode === 'raw'}
									onclick={() => (displayMode = 'raw')}>Raw</Button
								>
							</div>
						</div>
					</div>
					<Tabs.Content value="stream" class="tab-content"
						><ScrollArea class="result-scroll">
							<div class="timeline">
								{#each blocks as block (block.kind + block.index)}<StreamBlock
										{block}
										mode={displayMode}
										newest={block === blocks.at(-1)}
									/>{:else}<div class="empty">
										<span>∿</span>
										<h2>Awaiting protocol blocks</h2>
										<p>
											Run <code>example.ts</code> to emit the head expression and asynchronous tails.
										</p>
										<Button variant="outline" onclick={run}>Run example</Button>
									</div>{/each}
								{#if error}<div class="error-box">
										<strong>Worker error</strong>
										<pre>{error}</pre>
									</div>{/if}
								{#if running && blocks.length}<div class="pending-row">
										<i></i> waiting for the next async settlement…
									</div>{/if}
							</div>
						</ScrollArea></Tabs.Content
					>
					<Tabs.Content value="object" class="tab-content"
						><ScrollArea class="result-scroll"
							><div class="object-pane">
								{#if snapshot}<div class="object-note">
										<span>LIVE SNAPSHOT</span> Stable identities preserve aliases and cycles. For inspection
										only, the worker drains reconstructed client iterators into a capped visualization
										sidecar; raw blocks and server backpressure are unchanged.
									</div>
									<ObjectTree node={snapshot} />{:else}<div class="empty">
										<span>&#123; &#125;</span>
										<h2>No reconstructed graph</h2>
										<p>The object view updates after the head and every evaluated tail block.</p>
									</div>{/if}
							</div></ScrollArea
						></Tabs.Content
					>
		...
		<footer class="page-footer">
			<span><i class="green"></i> head evaluates in worker realm</span><span
				><i class="amber"></i> tails evaluate in arrival order</span
			><span class="spacer"></span><a
				href="https://github.com/sveltejs/devalue"
				target="_blank"
				rel="noreferrer">devalue ↗</a
			>
		</footer>
	</main>
</Tooltip.Provider>
