...
export function snapshotGraph(root: unknown, context = createSnapshotContext()): SnapshotNode {
	...
	function primitive(value: unknown): SnapshotNode | undefined {
		...
		switch (typeof value) {
			...
			case 'function':
				return { kind: 'function', value: `[Function ${(value as Function).name || 'anonymous'}]` };
		}
	}
	...
	function visit(value: unknown, depth: number): SnapshotNode {
		...
		if (value instanceof Promise) {
			const tracker = (
				value as Promise<unknown> & { __unevalTracker?: { state: string; value?: unknown } }
			).__unevalTracker;
			return tracker
				? {
						kind: 'Promise',
						id,
						state: tracker.state as 'pending' | 'fulfilled' | 'rejected',
						children:
							tracker.state === 'pending'
								? []
								: [
										{
											key: tracker.state === 'rejected' ? 'reason' : 'value',
											value: visit(tracker.value, depth + 1)
										}
									]
					}
		...
		if (value instanceof URLSearchParams)
			return { kind: 'URLSearchParams', id, value: value.toString() };
		if (value instanceof ArrayBuffer)
			return {
				kind: 'ArrayBuffer',
				id,
				meta: `${value.byteLength} bytes`,
				value: bytes(new Uint8Array(value))
			};
		if (ArrayBuffer.isView(value)) {
			...
			return {
				...
				id,
				meta: `${view.byteLength} bytes`,
				value: bytes(new Uint8Array(view.buffer, view.byteOffset, view.byteLength))
			};
		}
		if (value instanceof Map) {
			...
			for (const [key, item] of value) {
				...
				children.push({
					...
					value: {
						kind: 'entry',
						children: [
							{ key: 'key', value: visit(key, depth + 1) },
							{ key: 'value', value: visit(item, depth + 1) }
						]
					}
				});
				index++;
			...
			return {
				kind: 'Map',
				id,
				meta: `${value.size} entries`,
				children: truncate(children, value.size)
			};
		}
		if (value instanceof Set) {
			const children = Array.from(value)
				.slice(0, MAX_ITEMS)
				.map((item, index) => ({ key: String(index), value: visit(item, depth + 1) }));
			return {
				kind: 'Set',
				id,
				meta: `${value.size} values`,
				children: truncate(children, value.size)
			};
		}
		if (value instanceof Error)
			return {
				...
				value: value.message,
				children:
					value.cause === undefined ? [] : [{ key: 'cause', value: visit(value.cause, depth + 1) }]
			};
		if (Symbol.asyncIterator in object) {
			...
			const children = tracker.items.map((item, index) => ({
				key: `yield ${index}`,
				value: visit(item, depth + 1)
			}));
			if (tracker.itemCount > tracker.items.length)
				children.push({
					key: '…',
					value: {
						kind: 'truncated',
						value: `${tracker.itemCount - tracker.items.length} more yields`
					}
				});
			if (tracker.state === 'complete')
				children.push({ key: 'return', value: visit(tracker.returnValue, depth + 1) });
			else if (tracker.state === 'error')
				children.push({ key: 'error', value: visit(tracker.error, depth + 1) });
			return {
				kind: 'AsyncIterable',
				id,
				state: tracker.state,
				meta: `${tracker.itemCount} ${tracker.itemCount === 1 ? 'item' : 'items'}${tracker.itemCount > tracker.items.length ? ` · showing ${tracker.items.length}` : ''}`,
				children
			};
		}
		...
		for (const key of keys.slice(0, MAX_ITEMS)) {
			try {
				children.push({
					...
					value: visit(Reflect.get(object, key), depth + 1)
				});
			} catch (error) {
				children.push({
					key: String(key),
					value: { kind: 'Thrown', value: error instanceof Error ? error.message : String(error) }
				});
			}
		}
		return {
			kind: Array.isArray(value) ? 'Array' : value?.constructor?.name || 'Object',
			id,
			meta: Array.isArray(value) ? `${value.length} items` : undefined,
			children: truncate(children, keys.length)
		};
	}
...
function truncate(children: SnapshotEntry[], size: number): SnapshotEntry[] {
	return size > MAX_ITEMS
		? [...children, { key: '…', value: { kind: 'truncated', value: `${size - MAX_ITEMS} more` } }]
		: children;
}
