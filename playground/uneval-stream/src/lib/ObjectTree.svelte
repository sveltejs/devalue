<script lang="ts">
	...
	let stateVariant = $derived<'secondary' | 'destructive' | 'outline'>(
		node.state === 'pending' || node.state === 'streaming'
			? 'secondary'
			: node.state === 'rejected' || node.state === 'error'
				? 'destructive'
				: 'outline'
	);
</script>
	...
	<div class="tree-row">
		{#if expandable}
			<button
				class="tree-toggle"
				onclick={() => (open = !open)}
				aria-label={`${open ? 'Collapse' : 'Expand'} ${name}`}>{open ? '−' : '+'}</button
			>
		{:else}<span class="tree-spacer"></span>{/if}
		<span class="tree-key">{name}</span><span class="punct">:</span>
		<span class={`tree-value kind-${node.kind.toLowerCase()}`}
			>{node.ref ? `↗ #${node.ref}` : (node.value ?? node.kind)}</span
		>
		{#if node.id}<Badge variant="outline" class="identity">#{node.id}</Badge>{/if}
		{#if node.state}<Badge variant={stateVariant} class={`state state-${node.state}`}
				><i></i>{node.state}</Badge
			>{/if}
